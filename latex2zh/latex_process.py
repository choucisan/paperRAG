...
def patch_tex_bibliography(tex_path, bbl_name):
    ...
    if not os.path.exists(f'{bbl_name}.bbl'):
        return

    content = re.sub(r'(\\end{document})', f'\\bibliography{{{bbl_name}}}\n\\1', content, flags=re.IGNORECASE)

    with open(tex_path, 'w', encoding='utf-8') as f:
        f.write(content)

    print(f' Inserted \\bibliography{{{bbl_name}}} into {tex_path}')
