    ...
    def translate(self, text):
        ...
        while True:
            try:
                ...
                if hasattr(self, "is_error_request_frequency") and self.is_error_request_frequency(e):
                    time.sleep(0.5)
                else:
                    raise e
...
class LatexTranslator:
    ...
    def _translate_text_in_paragraph_latex(self, latex_original_paragraph):
        ...
        latex_translated_paragraph, nbad, ntotal = latex_process.recover_latex_objects(text_translated_paragraph, objs, tolerate_error=True)
        self.nbad += nbad
        self.ntotal += ntotal
    ...
    def translate_text_in_paragraph_latex_and_leading_brace(self, latex_original_paragraph):
        ...
        latex_translated_paragraph = latex_process.process_leading_level_brace(latex_translated_paragraph, self.translate_text_in_paragraph_latex_and_leading_brace)
        return latex_translated_paragraph
    ...
    def split_latex_to_paragraphs(self, latex):
        ...
        '''
        text, objs = latex_process.replace_latex_objects(latex)
        paragraphs_text = re.split(r'\n\n+', text)
        paragraphs_latex = [latex_process.recover_latex_objects(paragraph_text, objs)[0] for paragraph_text in paragraphs_text]
    ...
    def translate_full_latex(self, latex_original, make_complete=True, nocache=False):
        ...
        if self.add_cache:
            cache.remove_extra()
            self.hash_key = cache.deterministic_hash((latex_original, self.translator.engine, self.translator.language_from, self.translator.language_to, config.mularg_command_list))
        ...
        if self.complete:
            ...
            latex_original, tex_begin, tex_end = latex_process.split_latex_document(latex_original, r'\begin{document}', r'\end{document}')
        ...
        self.num = 'title'
        latex_translated = latex_process.process_specific_command(latex_translated, self.translate_text_in_paragraph_latex, 'title')

        latex_translated = latex_translated.replace('%', '\\%')
        latex_translated = latex_process.recover_special(latex_translated)
        latex_translated = latex_process.recover_accent(latex_translated)
